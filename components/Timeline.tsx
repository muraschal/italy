...
function TransportConnector({ from, to, event, dayKey, segmentColor, hoveredSegment, hoveredEventKey, hoveredLocationId, onHoverSegment, overrideToName, overrideFromId, overrideToId }: {
  ...
  const mode = event.transport as "walk" | "metro" | "car";
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-3 min-w-0">
        ...
        >
          ...
          {mode === "car" && fromLoc && toLoc && (
            <a
              href={`https://www.google.com/maps/dir/?api=1&origin=${fromLoc.coordinates.lat},${fromLoc.coordinates.lng}&destination=${toLoc.coordinates.lat},${toLoc.coordinates.lng}&travelmode=driving`}
              target="_blank"
            ...
            >
              ...
              <span className="text-[8px] font-semibold tracking-wide leading-none">ROUTE</span>
            </a>
          )}
        </div>
      </div>
              ...
              {(() => {
                const lastNonTransportIdx = (() => {
                  for (let j = day.events.length - 1; j >= 0; j--) {
                    if (day.events[j].transport !== "car" && day.events[j].transport !== "metro") return j;
                  }
                ...
                return day.events.map((event, i) => {
                  const isTransport = event.transport === "car" || event.transport === "metro";
                  const isMetro = event.transport === "metro";
                  const isCar = event.transport === "car";
