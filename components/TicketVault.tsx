          ...
          >
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <div className="flex items-start justify-between gap-2">
                ...
                <span
                  className={`shrink-0 text-[9px] uppercase tracking-[0.14em] font-semibold px-2 py-1 rounded-full border ${
                    ticket.confirmed
                      ? "bg-white/[0.06] text-text-secondary border-white/[0.06]"
                      : "bg-amber-400/10 text-amber-300/90 border-amber-400/20"
                  }`}
                >
                  {ticket.confirmed ? "Bestätigt" : "Offen"}
                </span>
              </div>
...
export default function TicketVault() {
  ...
  const confirmedCount = entries.filter(([, t]) => t.confirmed).length;
