...
function createTransportLabel(segment: RouteSegment, arcMid: [number, number], stageColor?: string) {
  const isCar = segment.transport === "car";
  const isMetro = segment.transport === "metro";
  const emoji = isMetro ? "🚇" : isCar ? "🚗" : "🚶";
  const text = segment.duration || (isMetro ? "Bahn" : isCar ? "Auto" : "");
  const bg = isMetro ? "rgba(0,120,212,0.85)" : isCar ? "rgba(100,70,200,0.8)" : stageColor ? `${stageColor}cc` : "rgba(60,120,60,0.7)";
...
/** Tighter fit = more zoom while keeping all markers in view; Tag 1 (0) tolerates closest zoom. */
...
export default function RouteMap({ activeDay: externalDay, onDayChange, compact, hoveredLocationId, hoveredEventKey, hoveredSegment, onHoverLocation, onHoverSegment }: RouteMapProps = {}) {
  ...
  const hasCar = segments.some((s) => s.transport === "car");
          ...
          >
            ...
            {(() => {
                ...
                if (metroRoute) {
                  ...
                  return (
                    <span key={`seg-${i}`}>
                        ...
                        return (
                          <span key={`metro-line-${li}`}>
                            ...
                            {lineSeg.stations.map((station, si) => {
                              ...
                              const isTransfer = station.transfer;
                              return (
                                <CircleMarker
                                  key={`station-${li}-${si}`}
                            ...
                            {(() => {
                              ...
                              return (
                                <Marker
                                  ...
                                  icon={L.divIcon({
                                    className: "custom-marker",
                                    html: `<div class="transport-badge" style="background:${lineColor}dd;font-weight:700;font-size:10px;letter-spacing:0.5px">${lineSeg.emoji} ${lineSeg.label}</div>`,
                                    iconSize: [1, 1],
                                    iconAnchor: [0, 0],
                                  })}
                                  interactive={false}
                      ...
                      {/* Walking connection after metro (z. B. Colle Aperto → Piazza Vecchia) */}
                      {(() => {
              ...
              const walkRoute = !isNonWalk && dayKey
                ...
                : null;
              const carRoute = isCar && dayKey
                ? carRoutes.find((r) => r.from === seg.from.id && r.to === seg.to.id && r.day === dayKey)
                : null;

              const routePoints: [number, number][] = walkRoute
                ? walkRoute.points as [number, number][]
                : carRoute
                  ? carRoute.points as [number, number][]
                  : (() => {
              ...
              const tLabel = seg.transport !== "none" ? createTransportLabel(seg, mid, (isNonWalk && !carRoute) ? undefined : stageColor) : null;
              ...
              const hasRealRoute = walkRoute || carRoute;
                ...
                <span key={`seg-${i}`}>
                  ...
                  <Polyline
                    ...
                    pathOptions={{
                      ...
                      weight: mainWeight,
                      opacity: mainOpacity,
                      dashArray: segHighlighted ? undefined : isCar ? "3 7" : walkRoute ? undefined : "6 5",
                    }}
                  />
